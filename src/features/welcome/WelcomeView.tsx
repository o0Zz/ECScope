        ...
        <div className="flex h-full flex-col items-center justify-center gap-4 text-muted-foreground">
            {status === "loading" && (
                <>
                    ...
                    <div className="text-center">
                        <h2 className="text-xl font-semibold text-foreground">Connecting to AWS…</h2>
            ...
            {(status === "idle" || status === "connected") && (
                <>
                    ...
                    <div className="text-center">
                        <h2 className="text-xl font-semibold text-foreground">Welcome to ECScope</h2>
                        {activeCluster ? (
                            ...
                            <p className="mt-1 text-sm">
                                {clusters.length} cluster{clusters.length > 1 ? "s" : ""} configured. Select a cluster
                                from the sidebar to get started.
                            </p>
                        ) : (
                            <p className="mt-1 text-sm">Select a cluster from the sidebar to get started.</p>
                        )}
                    </div>
                </>
            )}
